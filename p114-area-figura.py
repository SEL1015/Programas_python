...
#for de opciones a elegir 
for k,v in figuras.items():
    print(f'{k:<10} - Fórmula: {v["formula"]}')
#Menu donde pregunta el nombre de la figura hasta que de el valor adecuado 
while True:
    fig = input('\nElige una Figura: ').title() #primera matuscula y despues minusculas
    if fig in figuras : break
    print("Error: Figura no válida. Intenta de nuevo.")
area = 0

if fig == 'Circulo':
    r = int(input('Radio : '))#radio
